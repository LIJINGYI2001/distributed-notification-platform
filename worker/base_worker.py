...
from shared.metrics_store import update_metrics # filelock
...
def process_message(body: bytes, service_name: str):
    ...
    update_metrics(lambda metrics: metrics.__setitem__( # as update_metrics(fn:+= 1)
    ...
    if force_timeout:
        ...
        raise TimeoutError(f"Simulated timeout: processing exceeded {TIMEOUT_SECONDS} seconds")
...
def run_worker(service_name: str, queue_name: str): 
    ...
    def callback(ch, method, properties, body):
        ...
        try:
            ...
            # filelock: metrics_failed
            update_metrics(lambda metrics: metrics.__setitem__(
            ...
            if retry_count < max_retries:
                ...
                # filelock: metrics_retried
                update_metrics(lambda metrics: metrics.__setitem__(
                    "notifications_retried_total",
                    metrics["notifications_retried_total"] + 1
                ))
                ...
                print(f"[{service_name}] END -> DEAD LETTER")
                print("=" * 60)
